...
public class AppointmentService
{
    ...
    public AppointmentService(IGenericRepository<Appointment> appointmentRepository, IEmailService emailService)
    {
    ...
    public async Task<IEnumerable<Appointment>> GetAppointmentsForDoctorByDay(Guid doctorId, DateTime date)
    {
        ...
        var result = await _appointmentRepository.GetAsync(filter: a =>
                a.DoctorId == doctorId ,
            orderBy: appointments => appointments.OrderBy(a => a.AppointmentStartDate), isTracking: true,
            includeStringProperties: "Doctor,Patient");
        return result;
    }
    ...
    public async Task ChangeStateAppointment(Guid id, AppointmentState state, DateTime? newDate)
    {
        ...
        else if (state.Equals(AppointmentState.Rescheduled))
        {
            if (newDate.HasValue)
            {
                ...
                if (await DoctorAvailable(appointmentSearched.DoctorId, newDate.Value))
                {
